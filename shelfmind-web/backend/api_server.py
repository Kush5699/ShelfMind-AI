...
class ModelManager:
    ...
    def get_robust_embedding(self, image: Image.Image):
        ...
        return avg_emb, view_names, views
...
async def scan_single_product(image: UploadFile = File(...)):
    """Single product scan: rembg crop → robust DINOv2 embedding (10 augmented views)."""
    contents = await image.read()
    img = read_upload_image(contents)

    # 1. Auto-crop with rembg
    cropped = models.auto_crop_rembg(img)

    # 2. Robust embedding (10 augmented views)
    embedding, view_names, view_images = models.get_robust_embedding(cropped)

    # Convert view images to small base64 thumbnails
    view_thumbnails = []
    for v in view_images:
        thumb = v.copy()
        thumb.thumbnail((120, 120))
        view_thumbnails.append(image_to_base64(thumb))
    ...
    return {
        ...
        "augmentation_images": view_thumbnails,
...
async def scan_bulk_shelf(image: UploadFile = File(...), similarity_threshold: float = Form(0.65), confidence: float = Form(0.4)):
