...
# Database file location — use env var or resolve relative to this file
_db_base = os.environ.get("SHELFMIND_DATA_DIR", "")
if _db_base:
    DB_DIR = Path(_db_base)
else:
    # Local dev: parent.parent / data
    # Docker: /app/data (we detect Docker by checking if /app/api_server.py exists)
    _root = Path(__file__).resolve().parent.parent
    if (_root / "data").exists() or not Path("/app").exists():
        DB_DIR = _root / "data"
    else:
